...
public class ArtifactTest {
	...
	public void artifactTests() throws TskCoreException {
		...
		AnalysisResultAdded analysisResultAdded1 = abcTextFile.newAnalysisResult(new BlackboardArtifact.Type(BlackboardArtifact.ARTIFACT_TYPE.TSK_KEYWORD_HIT), 
																		new Score(Score.Significance.LOW, Score.Confidence.LOW), "Keyword hit found", "", "", attributes);
   
		assertEquals(analysisResultAdded1.getAnalysisResult().getScore().getSignificance().getId(), Score.Significance.LOW.getId());
		assertEquals(analysisResultAdded1.getAnalysisResult().getScore().getConfidence().getId(), Score.Confidence.LOW.getId());
		...
		AnalysisResultAdded analysisResultAdded2 = abcTextFile.newAnalysisResult(new BlackboardArtifact.Type(BlackboardArtifact.ARTIFACT_TYPE.TSK_KEYWORD_HIT), 
																	new Score(Score.Significance.MEDIUM, Score.Confidence.HIGH), "Keyword hit found", "", "", Collections.emptyList());
   
		
		// Add a 2nd analysis result to the same file
		AnalysisResultAdded analysisResultAdded3 = abcTextFile.newAnalysisResult(new BlackboardArtifact.Type(BlackboardArtifact.ARTIFACT_TYPE.TSK_INTERESTING_FILE_HIT), 
		...
		AnalysisResultAdded analysisResultAdded4 = abcTextFile.newAnalysisResult(new BlackboardArtifact.Type(BlackboardArtifact.ARTIFACT_TYPE.TSK_ENCRYPTION_DETECTED), 
		...
		assertEquals(ars.size(), 4);
		
		// verify the aggregate score - expect HIGH/HIGH - highest of the 4 results added
		...
		// delete an anlysis result #4
		Score newScore = caseDB.getBlackboard().deleteAnalysisResult(analysisResultAdded4.getAnalysisResult());
