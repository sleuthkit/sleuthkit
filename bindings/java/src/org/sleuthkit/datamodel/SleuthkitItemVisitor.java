...
/**
 * Interface for implementing a visitor pattern on all displayable items: Content 
 * implementations and blackboard artifacts.
 * 
 * Visitor implements an algorithm on the content and blackboard artifacts objects. 
 * The algorithm is completely decoupled from the data object.
 * The visitor pattern emulates double dispatch mechanism. It allows to act differently depending 
 * on the instance type, without need to test what the actual type is.
 * E.g.  it allows for processing an object hierarchy without using instanceof statements.
 * Generic type parameter T is a return type from the visit methods.
 *
...
public interface SleuthkitItemVisitor<T> {
	 ...
	 * Then, implement the specific visit methods
	 * for the objects on which the algorithm needs to act differently.
	 *
	 * @param <T> generic type, signifies the object type to be returned from visit()
	 */
