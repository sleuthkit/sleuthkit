    ...
    uint64_t * a_subvol, TSK_INUM_T * a_inum)
{
    ...
    if (a_vinum >= a_btrfs->virt2real_inums->size()) {
        btrfs_error(TSK_ERR_FS_ARG,
                "btrfs_inum_virt2real_map: invalid virtual inum: %" PRIdINUM, a_vinum);
        return false;
    }
    ...
    const TSK_INUM_T a_inum, TSK_INUM_T * a_vinum)
{
    ...
    if (inode_it == subvol_it->second.real2virt_inums.end()) {
        btrfs_error(TSK_ERR_FS_ARG,
                "btrfs_inum_real2virt_map: invalid real inum: %" PRIdINUM, a_inum);
        return false;
    }
 ...
 */
static BTRFS_TREENODE_RESULT
btrfs_extent_datawalk_get(BTRFS_EXTENT_DATAWALK * a_edw,
    BTRFS_EXTENT_DATA ** a_ed, TSK_DADDR_T * a_offset)
{
...
btrfs_inodewalk_invoke(BTRFS_INODEWALK * a_iw)
{
    ...
    if (!btrfs_inum_virt2real_map(a_iw->btrfs, current_vinum, &subvol, &inum)) {
        tsk_error_set_errstr2("btrfs_inodewalk_invoke: mapping inum of virtual inum: %" PRIdINUM, current_vinum);
    ...
    if (node_result == BTRFS_TREENODE_ERROR) {
        tsk_error_errstr2_concat("- btrfs_inodewalk_invoke: %s INODE_ITEM item of virtual inum: %" PRIdINUM,
                a_iw->node ? "stepping to current" : "loading", current_vinum);
        return (TSK_FS_META_FLAG_ENUM) 0;
    }
    if (node_result == BTRFS_TREENODE_NOT_FOUND) {
        btrfs_error(TSK_ERR_FS_INODE_COR, "btrfs_inodewalk_invoke: could not %s virtual inum: %" PRIdINUM,
    ...
    const TSK_FS_META_FLAG_ENUM a_flags, TSK_FS_META * a_meta)
{
    ...
    if (tsk_verbose)
        tsk_fprintf(stderr, "btrfs_inodewalk_fillmeta: Filling meta structure of inum: %" PRIdINUM "\n", current_vinum);
    ...
    if (tsk_verbose)
        tsk_fprintf(stderr, "btrfs_inodewalk_fillmeta: Checking EXTENT_DATA item(s) of inum: %" PRIdINUM "\n", current_vinum);
    ...
    for (;;) {
        ...
        if (a_meta->type == TSK_FS_META_TYPE_LNK) {
            if (!ed_is_raw) {
                btrfs_error(TSK_ERR_FS_INODE_COR,
                        "btrfs_inodewalk_fillmeta: non-raw symlink target of virtual inum: %" PRIdINUM, current_vinum);
                btrfs_extent_data_free(ed);
                btrfs_extent_datawalk_free(edw);
                return false;
            }
            ...
            if (!a_meta->link) {
                tsk_error_set_errstr2("btrfs_inodewalk_fillmeta: setting target of virtual inum: %" PRIdINUM, current_vinum);
                btrfs_extent_data_free(ed);
                btrfs_extent_datawalk_free(edw);
                return false;
            }
 ...
 */
static ssize_t
btrfs_datawalk_read(BTRFS_DATAWALK * a_dw, uint8_t * a_data,
    const size_t a_len)
{
    size_t written = 0;
    while (written < a_len) {
...
btrfs_load_attrs(TSK_FS_FILE * a_fs_file)
{
    ...
    if (tsk_verbose)
        tsk_fprintf(stderr, "btrfs_load_attrs: Loading attributes of inum: %" PRIdINUM "\n", meta->addr);
 ...
 */
static uint8_t
btrfs_istat(TSK_FS_INFO * a_fs, FILE * a_file, TSK_INUM_T a_inum,
    TSK_DADDR_T a_numblock, int32_t a_sec_skew)
{
    ...
    tsk_fprintf(a_file, "Size: %" PRIuOFF "\n", meta->size);
    ...
    TSK_FS_TYPE_ENUM ftype, uint8_t test)
{
    ...
    if (tsk_verbose)
        tsk_fprintf(stderr, "btrfs_open: SB mirror: %d, node size: %" PRId32 ", block size: %d, blocks: %" PRIdDADDR ", virtual inodes: %" PRIdINUM ", subvols: %zd, label: '%s'\n",
