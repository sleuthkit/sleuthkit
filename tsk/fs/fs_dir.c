    ...
    TSK_FS_DIR_WALK_CB a_action, void *a_ptr)
{
    ...
    for (i = 0; i < fs_dir->names_used; i++) {
         ...
         */
        if ((((fs_file->name->type == TSK_FS_NAME_TYPE_DIR) || (fs_file->name->type == TSK_FS_NAME_TYPE_VIRT_DIR))
    ...
    void *a_ptr)
{
    ...
    if (a_fs_file->meta) {
        ...
        if (((a_fs_file->meta->type == TSK_FS_META_TYPE_DIR) || (a_fs_file->meta->type == TSK_FS_META_TYPE_VIRT_DIR))
