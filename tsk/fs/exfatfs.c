...
exfatfs_get_alloc_bitmap(FATFS_INFO *a_fatfs)
{
    ...
    while (current_sector < last_sector_of_data_area && num_bitmaps_found < a_fatfs->numfat) {
        ...
        for (i = 0; i < a_fatfs->ssize; i += EXFAT_DIR_ENTRY_SIZE_IN_BYTES) {
            dir_entry = (uint8_t*)&(sector_buf[i]); 

            /* The type of the directory entry is encoded in the first byte 
             * of the entry. See TSK_FS_EXFAT_DIR_ENTRY_TYPE_ENUM. */ 
            if (dir_entry[0] == TSK_FS_EXFAT_DIR_ENTRY_TYPE_ALLOC_BITMAP) {
...
int 
exfatfs_is_clust_alloc(FATFS_INFO *a_fatfs, TSK_DADDR_T cluster_addr)
{
    const char *func_name = "exfatfs_is_clust_alloc";
    TSK_FS_INFO *fs = &(a_fatfs->fs_info);
    TSK_DADDR_T bitmap_byte_offset = (cluster_addr - 2) / 8;
    ...
    return ((1 << (cluster_addr % 8)) & bitmap_byte[0]);
}
