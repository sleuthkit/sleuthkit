    ...
    TSK_INUM_T a_attrinum, const NTFS_ATTRLIST_MAP * a_attr_map, TSK_STACK * a_seen_inum_list)
{
    ...
    for (attr = a_attrseq; ((uintptr_t) attr >= (uintptr_t) a_attrseq)
        ...
        if (attr->res == NTFS_MFT_RES) {
            ...
            if (tsk_fs_attr_set_str_offset(fs_file, fs_attr,
                                    name, (TSK_FS_ATTR_TYPE_ENUM) type,
                                    id_new, (void *) ((uintptr_t) attr + tsk_getu16(fs->endian, attr->c.r.soff)),
                                    tsk_getu32(fs->endian, attr->c.r.ssize), contentOffset )) {
                tsk_error_errstr2_concat("- proc_attrseq");
                return TSK_ERR;
            }
