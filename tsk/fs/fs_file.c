 ...
 * @returns 0 on success or 1 on error
 */
extern uint8_t tsk_fs_file_hash_calc(TSK_FS_FILE * a_fs_file, TSK_FS_HASH_RESULTS * a_hash_results, TSK_FS_HASH_ENUM a_flags){
	...
	if(a_flags & TSK_FS_HASH_MD5){
		TSK_MD5_Init(&(hash_data.md5_context));
	}
	if(a_flags & TSK_FS_HASH_SHA1){
