...
#define CACHE_AGE   1000
    ...
    if ((a_len + (a_off % 512)) > TSK_IMG_INFO_CACHE_LEN) {
        ...
        if (a_len % a_img_info->sector_size) {
            char *buf2 = a_buf;
            // TODO: why is len2 redefined here?
            size_t len2;
    ...
    for( cache_index = 0;
         cache_index < TSK_IMG_INFO_CACHE_NUM;
         cache_index++ ) {
        ...
        if (a_img_info->cache_len[ cache_index ] > 0) {
            ...
            if( ( read_count == 0 )
             ...
             && ( a_img_info->cache_off[ cache_index ] + a_img_info->cache_len[ cache_index ] >= a_off + len2 ) ) {
                ...
                // We found it...
                memcpy(a_buf,
                    &a_img_info->cache[ cache_index ][a_off - a_img_info->cache_off[ cache_index ]], len2);
    ...
    if (read_count == 0) {
        ...
        if( ( a_img_info->cache_off[ cache_next ] + read_size ) > a_img_info->size ) {
            read_size = (size_t) ( a_img_info->size - a_img_info->cache_off[ cache_next ] );
        }

        read_count = a_img_info->read(
                      a_img_info,
                      a_img_info->cache_off[cache_next],
                      a_img_info->cache[cache_next],
                      read_size );
