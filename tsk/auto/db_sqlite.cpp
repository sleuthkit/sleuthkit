                              ...
                              const string& md5, const string& deviceId)
{
    ...
    deviceIdStr << deviceId;
#endif
	sql = sqlite3_mprintf("INSERT INTO data_source_info (obj_id, device_id, time_zone) VALUES (%lld, '%s', '%s');", objId,
                     ...
                     int64_t& objId, int64_t dataSourceObjId)
{
	...
	const char* root_folder = (firstslash == string::npos)
		                          ? escaped_path
		                          : escaped_path_str.substr(0, firstslash).c_str();
	if (addMACTimeEvent(zSQL, dataSourceObjId, objId, mtime, 4, full_description, escaped_path, root_folder)
		|| addMACTimeEvent(zSQL, dataSourceObjId, objId, atime, 5, full_description, escaped_path, root_folder)
		|| addMACTimeEvent(zSQL, dataSourceObjId, objId, crtime, 6, full_description, escaped_path, root_folder)
		|| addMACTimeEvent(zSQL, dataSourceObjId, objId, ctime, 7, full_description, escaped_path, root_folder))
	{
		free(name);
		free(escaped_path);
		sqlite3_free(zSQL);
