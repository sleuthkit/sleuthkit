    ...
    for (const auto entry : timeMap)
    {
        ...
        if (event_description_id == -1)
        {
            ...
            if (attempt_exec(descriptionSql,
                             "TskDbSqlite::addMACTimeEvents: Error adding filesystem event to tsk_event table: %s\n")
            )
            {
        ...
        if (attempt_exec(
                eventSql, "TskDbSqlite::addMACTimeEvents: Error adding filesystem event to tsk_event table: %s\n")
        )
        {
