...
bool TskDb::getParentPathAndName(const char *path, const char **ret_parent_path, const char **ret_name){
    ...
    if (path_len >= MAX_PATH_LENGTH) {
        ...
        tsk_error_set_errstr("TskDb::getParentPathAndName: Path is too long. Length = %d, Max length = %d", (int)path_len, MAX_PATH_LENGTH);
    ...
    if (path[0] != '/') {
        snprintf(parent_path, MAX_PATH_LENGTH, "%s", "/");
    }
