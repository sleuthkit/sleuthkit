...
int TskDbSqlite::attempt(int resultCode, int expectedResultCode, const char *errfmt)
{
...
int TskDbSqlite::attempt(int resultCode, const char *errfmt)
{
...
int TskDbSqlite::attempt_exec(const char *sql, int (*callback)(void*,int,char**,char**), void* callback_arg, const char *errfmt)
{
    char * errmsg;

    if (!m_db) return 1;

    if (sqlite3_exec(m_db, sql, callback, callback_arg, &errmsg) != SQLITE_OK) {
        tsk_error_reset();
        tsk_error_set_errno(TSK_ERR_AUTO_DB);
...
int TskDbSqlite::attempt_exec(const char *sql, const char *errfmt)
{
    return attempt_exec(sql, NULL, NULL, errfmt);
}


int TskDbSqlite::prepare_stmt(const char *sql, sqlite3_stmt **ppStmt)
{
...
int TskDbSqlite::addObject(DB_OBJECT_TYPES type, int64_t parObjId, int64_t & objId)
{   
    char stmt[1024];
    ...
    snprintf(stmt, 1024,
        ...
        parObjId, type);
    if (attempt_exec(stmt,
        "Error adding data to tsk_objects table: %s\n")) {
        return 1;
    }
...
int TskDbSqlite::initialize()
{
    ...
    if (attempt_exec("CREATE TABLE tsk_objects (obj_id INTEGER PRIMARY KEY, par_obj_id INTEGER, type INTEGER);",
                     "Error creating tsk_objects table: %s\n") ||
        
        attempt_exec("CREATE TABLE tsk_image_info (obj_id INTEGER, type INTEGER, ssize INTEGER);",
                     "Error creating tsk_image_info table: %s\n") ||

        attempt_exec("CREATE TABLE tsk_image_names (obj_id INTEGER, name TEXT, sequence INTEGER);",
                     "Error creating tsk_image_names table: %s\n") ||

        attempt_exec("CREATE TABLE tsk_vs_info (obj_id INTEGER, vs_type INTEGER, img_offset INTEGER NOT NULL, block_size INTEGER NOT NULL);",
                     "Error creating tsk_vs_info table: %s\n") ||
         
        attempt_exec("CREATE TABLE tsk_vs_parts (obj_id INTEGER PRIMARY KEY, addr INTEGER, start INTEGER NOT NULL, length INTEGER NOT NULL, desc TEXT, flags INTEGER);",
                     "Error creating tsk_vol_info table: %s\n") ||
        
        attempt_exec("CREATE TABLE tsk_fs_info (obj_id INTEGER PRIMARY KEY, img_offset INTEGER, fs_type INTEGER, block_size INTEGER, block_count INTEGER, root_inum INTEGER, first_inum INTEGER, last_inum INTEGER);",
                     "Error creating tsk_fs_info table: %s\n") ||
        
        attempt_exec("CREATE TABLE tsk_files (fs_obj_id INTEGER NOT NULL, obj_id INTEGER NOT NULL UNIQUE, attr_type INTEGER, attr_id INTEGER, name TEXT NOT NULL, meta_addr INTEGER, type INTEGER, has_layout INTEGER, has_path INTEGER, dir_type INTEGER, meta_type INTEGER, dir_flags INTEGER, meta_flags INTEGER, size INTEGER, ctime INTEGER, crtime INTEGER, atime INTEGER, mtime INTEGER, mode INTEGER, uid INTEGER, gid INTEGER);",
                     "Error creating tsk_fs_files table: %s\n") ||
        
        attempt_exec("CREATE TABLE tsk_files_path (obj_id INTEGER, path TEXT)",
                     "Error creating tsk_files_path table: %s\n") ||
        
        attempt_exec("CREATE TABLE tsk_files_derived (obj_id INTEGER UNIQUE, derived_id INTEGER, rederive TEXT)",
                     "Error creating tsk_files_derived table: %s\n") ||
        
        attempt_exec("CREATE TABLE tsk_files_derived_method (derived_id INTEGER PRIMARY KEY, tool_name TEXT, tool_version TEXT, other TEXT)",
                     "Error creating tsk_files_derived_method table: %s\n")) {
        return 1;
    }

    if (m_blkMapFlag) {
        if (attempt_exec("CREATE TABLE tsk_file_layout (fs_id INTEGER NOT NULL, byte_start INTEGER NOT NULL, byte_len INTEGER NOT NULL, obj_id);",
                         "Error creating tsk_fs_blocks table: %s\n")) {
            return 1;
        }
    }

    if (prepare_stmt("SELECT obj_id FROM tsk_files WHERE meta_addr IS ? AND fs_obj_id IS ?",
        &m_selectFileIdByMetaAddr)) {
            return 1;
    }
    
    if (createIndexes()) return 1;
...
{
    return attempt_exec("CREATE INDEX parObjId ON tsk_objects(par_obj_id);",
 ...
 */
int TskDbSqlite::open()
{

    if (m_utf8) {
        if (attempt(sqlite3_open(m_dbFilePathUtf8, &m_db), "Can't open database: %s\n")) {
            sqlite3_close(m_db);
            return 1;
        }
    }  else {
        if (attempt(sqlite3_open16(m_dbFilePath, &m_db), "Can't open database: %s\n")) {
            sqlite3_close(m_db);
            return 1;
        }
    }
...
int TskDbSqlite::addImageInfo(int type, int size, int64_t & objId)
{
    char stmt[1024];
...
int TskDbSqlite::addImageName(int64_t objId, char const * imgName, int sequence)
{
    char stmt[1024];
...
int TskDbSqlite::addVsInfo(const TSK_VS_INFO * vs_info, int64_t parObjId, int64_t & objId)
{
    char stmt[1024];

    if (addObject(DB_OBJECT_TYPE_VS, parObjId, objId)) return 1;
 ...
 */
int TskDbSqlite::addVolumeInfo(const TSK_VS_PART_INFO * vs_part, int64_t parObjId, int64_t & objId)
{
    char stmt[1024];

    if (addObject(DB_OBJECT_TYPE_VOL, parObjId, objId)) return 1;

    snprintf(stmt, 1024,
        "INSERT INTO tsk_vs_parts (obj_id, addr, start, length, desc, flags)"
        "VALUES (%lld, %" PRIuPNUM ",%" PRIuOFF ",%" PRIuOFF ",'%s',%d)",
        objId, (int) vs_part->addr, vs_part->start, vs_part->len, vs_part->desc, vs_part->flags);

    return attempt_exec(stmt,
        "Error adding data to tsk_vs_parts table: %s\n");
}

int TskDbSqlite::addFsInfo(const TSK_FS_INFO * fs_info, int64_t parObjId, int64_t & objId)
{
    char stmt[1024];

    if (addObject(DB_OBJECT_TYPE_FS, parObjId, objId)) return 1;
    ...
    snprintf(stmt, 1024,
        ...
        objId, fs_info->offset, (int) fs_info->ftype, fs_info->block_size, fs_info->block_count,
...
int TskDbSqlite::addFsFile(TSK_FS_FILE * fs_file,
    const TSK_FS_ATTR * fs_attr, const char *path, int64_t fsObjId, int64_t & objId)
{
    int64_t parObjId;
    ...
    if (fs_file->fs_info->root_inum == fs_file->name->meta_addr) {
        ...
        if (attempt(sqlite3_reset(m_selectFileIdByMetaAddr),
            ...
            attempt(sqlite3_bind_int64(m_selectFileIdByMetaAddr, 1, fs_file->name->par_addr),
                    "Error binding meta_addr to statment: %s (result code %d)\n") ||
            attempt(sqlite3_bind_int64(m_selectFileIdByMetaAddr, 2, fsObjId),
                    "Error binding fs_obj_id to statment: %s (result code %d)\n") ||
            attempt(sqlite3_step(m_selectFileIdByMetaAddr),
                    SQLITE_ROW,
 ...
 */
int TskDbSqlite::addFile(TSK_FS_FILE * fs_file,
    const TSK_FS_ATTR * fs_attr, const char *path, int64_t fsObjId, int64_t parObjId, int64_t & objId)
{
    ...
    if (addObject(DB_OBJECT_TYPE_FILE, parObjId, objId)) return 1;
    ...
    snprintf(foo, 1024,
        ...
        size,
        crtime, ctime, atime, mtime, meta_mode, gid, uid);
...
int TskDbSqlite::savepoint(const char * name)
{
    char buff[1024];

    snprintf(buff, 1024, "SAVEPOINT %s", name);

    return attempt_exec(buff,
        "Error setting savepoint: %s\n");
}

int TskDbSqlite::rollbackSavepoint(const char * name)
{
    char buff[1024];
    ...
    return attempt_exec(buff,
        "Error rolling back savepoint: %s\n");
}
 ...
 * @returns 1 on error
 */
int TskDbSqlite::addFsBlockInfo(int64_t a_fsObjId, int64_t a_fileObjId, uint64_t a_byteStart, uint64_t a_byteLen)
{
    char foo[1024];
 ...
 */
int TskDbSqlite::addCarvedFileInfo(int fsObjId, const char *fileName, uint64_t size, int64_t & objId)
{
    char foo[1024];
    ...
    if (addObject(DB_OBJECT_TYPE_FILE, fsObjId, objId)) return 1;
    ...
    snprintf(foo, 1024,
        ...
        name,
        TSK_FS_NAME_TYPE_REG, TSK_FS_META_TYPE_REG, TSK_FS_NAME_FLAG_UNALLOC, TSK_FS_NAME_FLAG_UNALLOC,
        size);
