...
endsWith( const TSK_TCHAR *str, const TSK_TCHAR *suffix )
{
    ...
    if ( TSTRLEN(str) >= TSTRLEN(suffix) ) {
        return ( TSTRICMP(&str[TSTRLEN(str)-TSTRLEN(suffix)], suffix) == 0 );
    }
    return 0;
}
...
getSegmentName(const TSK_TCHAR *a_startingName, int a_segmentNumber) 
{
    ...
    TSK_TCHAR *newName = (TSK_TCHAR *)tsk_malloc( (nameLen+32) * sizeof(TSK_TCHAR) );
    ...
    if ( endsWith(a_startingName, _TSK_T(".dmg")) ) {
        TSNPRINTF(newName+nameLen-3, 35, _TSK_T("%03d.dmgpart"), a_segmentNumber);
        return newName;
    }
    ...
    if ( endsWith(a_startingName, _TSK_T(".001")) ||
        ...
        TSNPRINTF(newName+nameLen-3, 35, _TSK_T("%03d"), a_segmentNumber);
        return newName;
    }
    ...
    if ( endsWith(a_startingName, _TSK_T(".000")) ||
        ...
        TSNPRINTF(newName+nameLen-3, 35, _TSK_T("%03d"), a_segmentNumber-1);
        return newName;
    }
    ...
    if ( endsWith(a_startingName, _TSK_T(".01")) ||
        ...
        TSNPRINTF(newName+nameLen-2, 34, _TSK_T("%02d"), a_segmentNumber);
        return newName;
    }
    ...
    if ( endsWith(a_startingName, _TSK_T(".00")) ||
        ...
        TSNPRINTF(newName+nameLen-2, 34, _TSK_T("%02d"), a_segmentNumber-1);
        return newName;
    }
...
tsk_img_findFiles(const TSK_TCHAR *a_startingName, int *a_numFound) 
{
    ...
    *a_numFound = 0;
    ...
    while ( (nextName = getSegmentName(a_startingName, fileCount+1)) != NULL ) {
        ...
        if (fileCount == 1)
            ...
            tmpNames = (TSK_TCHAR **)tsk_realloc(retNames, fileCount * sizeof(TSK_TCHAR *));
        if (tmpNames == NULL) {
            if (retNames != NULL)
                free(retNames);
            return NULL;
