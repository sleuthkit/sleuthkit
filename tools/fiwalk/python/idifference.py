...
ignore_filenames = set(['$FAT1','$FAT2'])
...
class DiskState:
    ...
    def __init__(self,notimeline=False):
        self.new_sha1s  = dict() # maps from sha1 -> fi
    ...
    def process_fi(self,fi):
        ...
        if fi.filename() in ignore_filenames:
            return 

        # Remember the file for the next generation
        self.new_sha1s[fi.sha1()] = fi
    ...
    def output_archive(self,imagefile=None,tarname=None,zipname=None):
        ...
        for fi in to_archive:
            ...
            if zfile:
                info = zipfile.ZipInfo(filename,datetime.datetime.fromtimestamp(fi.mtime()).utctimetuple())
...
if __name__=="__main__":
    ...
    s = DiskState(notimeline=options.notimeline)
    for infile in args:
